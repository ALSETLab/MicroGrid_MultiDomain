...
model GTPlant_GGov1_PU "ThermoPower Gas Turbine Plant Example with variable shaft speed (pu)"
  ...
  Turbine turbine(
    ...
    Tc=Tc,
    Teng=Teng,
    Dm=Dm) annotation (Placement(transformation(extent={{-40,-20},{
            20,40}})));
...
equation
  connect(valvePosition, turbine.FSR) annotation (Line(points={{-146,4},{-120,4},
          {-120,-11.6429},{-42.3571,-11.6429}},  color={0,0,127}));
  connect(gain.y, turbine.PELEC) annotation (Line(points={{-51.4,10},{-41.9286,
          10},{-41.9286,10.2143}},color={0,0,127}));
  connect(add.y, gain.u) annotation (Line(points={{-75,10},{-65.2,10}}, color={0,0,127}));
  connect(add.u2, turbine.FSR) annotation (Line(points={{-98,4},{-120,4},{-120,
          -11.6429},{-42.3571,-11.6429}},color={0,0,127}));
  connect(const.y, add.u1) annotation (Line(points={{-119.3,25},{-110,25},{-110,16},{-98,16}}, color={0,0,127}));
  connect(speedRef, turbine.SPEED) annotation (Line(points={{-78,44},{-58,44},{
          -58,31.4286},{-42.1429,31.4286}},color={0,0,127}));
  connect(turbine.PMECH, Pmech) annotation (Line(points={{22.5714,10},{60,10},{
          60,0},{146,0}},color={0,0,127}));
