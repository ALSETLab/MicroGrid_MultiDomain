...
model PIDGovernor2A "GE General GGOV Governor Frequency Controller, PID Implementation 2A"
...
public
  ...
  Modelica.Blocks.Nonlinear.Limiter V(uMax=Vmax, uMin=Vmin) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={109,53})));
  Modelica.Blocks.Interfaces.RealInput PSP annotation (Placement(transformation(extent={{-148,-6},{-122,20}}), iconTransformation(extent={{-168,-18},{-134,16}})));
...
76},{109,76},{109,103}},    color={0,0,127}));
  ...
  connect(r_.u, PSP) annotation (Line(points={{-113.4,7},{-126,7},{-135,7}}, color={0,0,127}));
  connect(add2.u1, r_.y) annotation (Line(points={{-85.2,1.6},{-92,1.6},{-92,7},{-97.3,7}}, color={0,0,127}));
  connect(Vstr_set.y, rSELECT.ValveStroke) annotation (Line(points={{-4.9,
-103},{10,-103},{10,-77},{-52,-77}}, color={0,0,127}));
  annotation (
...
", info="<html>
<p><span style=\"font-family: Arial,sans-serif;\">Similar to PIDGovernor1B but with a different implementation of the integral mode of the PID controller.</span></p>
</html>"));
end PIDGovernor2A;
