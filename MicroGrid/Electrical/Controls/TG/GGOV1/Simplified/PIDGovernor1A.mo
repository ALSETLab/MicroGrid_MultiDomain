within MicroGrid.Electrical.Controls.TG.GGOV1.Simplified;
model PIDGovernor1A "GE General GGOV Governor Frequency Controller, PID Implementation 1A"
  ...
  parameter Real Wfnl "Non load fuel flow";
  ...
  Modelica.Blocks.Nonlinear.DeadZone deadZone(uMax=db)   annotation (Placement(transformation(extent={{-14,4},{6,24}})));
  ...
  Modelica.Blocks.Math.Add LoadlimiterPI2(k1=-1) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={123,-29})));
  OpenIPSL.Electrical.Controls.PSSE.TG.BaseClasses.GGOV1.R_select rSELECT(Rselect=Rselect) annotation (Placement(transformation(
        extent={{-15,-22},{15,22}},
        rotation=90,
        origin={-52,-43})));
protected
...
public
  Modelica.Blocks.Interfaces.RealInput PELEC "Machine electrical power (pu)" annotation (Placement(transformation(extent={{-182,-118},{-154,-90}}), iconTransformation(extent={{-168,-98},{-134,-64}})));
  Modelica.Blocks.Interfaces.RealInput P_REF annotation (Placement(transformation(extent={{-156,28},{-132,52}}), iconTransformation(extent={{-168,-18},{-134,16}})));
  Modelica.Blocks.Interfaces.RealInput SPEED "Machine speed deviation from nominal (pu)"
    annotation (Placement(transformation(extent={{-102,28},{-76,54}}), iconTransformation(extent={{-168,64},{-134,98}})));
  Modelica.Blocks.Interfaces.RealInput GOVOUT1 "Governor Output before Limiter" annotation (Placement(transformation(
        extent={{-16,-16},{16,16}},
        rotation=90,
  ...
  Modelica.Blocks.Sources.Constant Vstr_set(k=0) annotation (Placement(transformation(extent={{-18,-92},{4,-70}})));
  Modelica.Blocks.Sources.Constant Pmw_set(k=Pmwset) "Supervisory Load Controller Setpoint," annotation (Placement(transformation(extent={{-130,-58},{-108,-36}})));
initial equation
  Pe0 = PELEC;
  Pmech0 = PELEC;
  Pmwset = Pe0;
...
equation
  ...
  connect(s0.y, rSELECT.Pelect) annotation (Line(
      points={{-105,-82},{-69.6,-82},{-69.6,-61}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(rSELECT.y, r.u) annotation (Line(
      points={{-52,-26.5},{-51.72,-26.5},{-51.72,-21.4},{-53,-21.4}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(GOVOUT1, rSELECT.GovernorOutput) annotation (Line(points={{30,-104},
{30,-61},{-34.4,-61}},  color={0,0,127}));
  connect(FSRN, GovernorPID.y) annotation (Line(points={{145,15},{125.1,15}}, color={0,0,127}));
  connect(LoadlimiterPI2.u2, rSELECT.GovernorOutput) annotation (Line(points={{129,-32},
{136,-32},{136,-60},{30,-60},{30,-61},{-34.4,-61}},   color={0,0,127}));
  connect(Vstr_set.y, rSELECT.ValveStroke) annotation (Line(points={{5.1,-81},
{20,-81},{20,-61},{-52,-61}},     color={0,0,127}));
  connect(Pmw_set.y, add3.u1) annotation (Line(points={{-106.9,-47},{-95.6,-47},{-95.6,-41.2}}, color={0,0,127}));
  annotation (
...
", info="<html>
<p>This PID governor does not include the valve position limiter. It is meant to be used together with the load and acceleration limiters.</p>
</html>"));
end PIDGovernor1A;
