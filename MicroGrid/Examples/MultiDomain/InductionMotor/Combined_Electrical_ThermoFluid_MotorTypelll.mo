...
model Combined_Electrical_ThermoFluid_MotorTypelll
  ...
  Modelica.Blocks.Sources.RealExpression Water_Flow_Ref(y=243.912)
    annotation (Placement(transformation(extent={{-182,-18},{-162,2}})));
  ...
  Electrical.MultiDomain.InductionMotor.ThreePhase.PSAT.MotorTypeIII_MultiDomain_Full_TESTORINI
    MotorTypelll(
    V_b=380,
    P_0(displayUnit="MW") = 1000000,
    Q_0(displayUnit="MVA") = 0,
    Sup=1,
    Rs=0.013,
    Xs=0.14,
    Rr1=0.009,
...
equation
  ...
  connect(Water_Flow_Ref.y, pump_controller.m_flow_ref)
    annotation (Line(points={{-161,-8},{-152,-8}},
                                                 color={0,0,127}));
  ...
  connect(fault.p, Grid.p) annotation (Line(points={{-143.667,56},{-150,
          56},{-150,32},{-158,32}},
                              color={0,0,255}));
  connect(Sensor_flow.m_flow, pump_controller.m_flow) annotation (Line(points={{
          138,-39},{138,-78},{-160,-78},{-160,-32},{-152,-32}}, color={0,0,127}));
  connect(aC_2_DC_and_DC_2_AC.n, MotorTypelll.p)
    annotation (Line(points={{-56,32},{-46,32}}, color={0,0,255}));
  connect(MotorTypelll.flange, torqueSensor.flange_a)
    annotation (Line(points={{-26,32},{-16,32}}, color={0,0,0}));
  connect(torqueSensor.tau, MotorTypelll.mech_torque) annotation (Line(points={{-14,21},
          {-14,12},{-30,12},{-30,20}},         color={0,0,127}));
  connect(volts_Hertz_Control.we, MotorTypelll.we)
    annotation (Line(points={{-54,-30},{-36,-30},{-36,20}}, color={0,0,127}));
  connect(MotorTypelll.wr, volts_Hertz_Control.motor_speed)
    annotation (Line(points={{-42,20},{-42,-10},{-54,-10}}, color={0,0,127}));
...
end Combined_Electrical_ThermoFluid_MotorTypelll;
